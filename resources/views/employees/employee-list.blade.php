            ...
            <div class="row">
                <div class="col-xl-12 col-sm-12 col-12">
                    <div class="breadcrumb-path mb-4">
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}"><img src="assets/img/dash.png"
                ...
                <div class="col-xl-12 col-sm-12 col-12 mb-4">
                    <div class="row">
                        <div class="col-xl-10 col-sm-8 col-12 ">
                            <label class="employee_count">{{ $employees->count() }}
                                People{{ $employees->count() > 0 ? 's' : '' }}</label>
                        </div>
                                ...
                                <tbody>
                                    @forelse($employees as $e)
                                        <tr>
                                            <td>
                                                <div class="table-img">
                                                    <a href="{{ route('profileEmployee', $e->id) }}">
                                                        <img src="{{ \App\Models\User::find($e->id)->profile_photo_url }}"
                                                            alt="profile"
                                                            class="img-table" /><label>{{ $e->name }}</label>
                                                    </a>
                                                </div>
                                            </td>
                                            <td>
                                                <label class="action_label">{{ \App\Models\Team::teamChief($e->id) }}
                                                </label>
                                            </td>
                                            <td>
                                            ...
                                            <td><label>{{ \App\Models\Team::ChiefTeam($e->id) == $e->name ? 'Team Lead' : 'Member' }}</label>
                                            </td>
                                            <td class="tab-select">
                                                <a class="btn btn-{{ $e->profile == 'active' ? 'info' : 'danger' }}"
                                                    href="{{ route('employeeStatus', $e->id) }}">{{ $e->profile == 'active' ? 'Active' : 'None' }}</a>
                                            </td>
                                        </tr>
                                    @empty
                                    @endforelse
